...
/*
get_clock(&is->vidclk):
获取到的实际上是:最后一帧的pts 加上 从处理最后一帧开始到现在的时间,具体参考set_clock_at
和get_clock的代码
c->pts_drift=最后一帧的pts-从处理最后一帧时间
clock=c->pts_drift+现在的时候
get_clock(&is->vidclk) ==is->vidclk.pts, 
...
{
    if (is->paused) {
        // last_updated 记录了上一帧视频图像显示时的系统时钟，
        // av_gettime() - last_updated得到的结果刚好是pause这段时间间隔，  
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            if (lastvp->serial != vp->serial) {
                //lastvp->serial != vp->serial 
                //说明SEEK过,重新调整frame_timer  
            ...
            /*通过pts计算duration，duration是上一帧videoframe的持续时间，
            当前帧的pts减去上一帧的pts*/
            ...
            if (!isnan(vp->pts)) {    
                /*更新视频的clock，将当前帧的pts和当前系统的时间保存起来，
                这2个数据将和audio  clock的pts 和系统时间一起计算delay*/
                update_video_pts(is, vp->pts, vp->pos, vp->serial);
            }
               
            SDL_UnlockMutex(is->pictq.mutex);
            /*frame_timer+duration 当前帧的播放时间+当前帧的持续时间=下一帧的播放时间
            time > is->frame_timer + duration  当前时间>下一帧的播放时间,
            来不及播放本帧,下一帧的播放时间已经到了,说明当前帧可以丢弃了*/
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    while (len > 0) {
        if (is->audio_buf_index >= is->audio_buf_size) {
           audio_size = audio_decode_frame(is);// 音频解码
           if (audio_size < 0) {
...
static int audio_open(void *opaque, int64_t wanted_channel_layout, 
    int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
